# 🎨 하루일본어 어휘 카드 V4 이미지 파이프라인 생성 계획서
> **문서 버전**: v4.0.0  
> **최종 수정일**: 2026년 8월 2일  
...
하루일본어(Haru Nihonggo) 앱의 JLPT N5~N1 어휘 8,424개 전체를 대상으로, **노이즈 0%의 최상급 2D 애니메이션 일러스트 플래시카드(V4 시리즈)**를 완전 수집·생성하는 자동화 파이프라인 기획서입니다.

---

## 2. 수집 현황 (Current Progress)

| 구분 | 수량 | 비율 | 비고 |
| :--- | :--- | :--- | :--- |
| **전체 어휘 목표** | **8,424개** | 100.0% | N5, N4, N3, N2, N1 |
| **생성 완료** | **4,298개** | **51.0%** | `assets/images/words_v4_gemini/` 저장 완료 |
| **남은 생성 대상** | **4,126개** | **49.0%** | Vertex AI ADC 파이프라인 수집 대상 |
...
- **API 모델**: `gemini-2.5-flash-image` / `gemini-3.1-flash-image`
...
- **비용 안전 정책**:
  ...
  - 남은 4,126개 전량 생성 시 소요 비용: 약 **₩120,000 ~ ₩160,000원** (장당 약 30~40원)
  - **추가 비용 및 사용자 개인 결제 부담금 0원** (무료 크레딧 범위 내 100% 충당)

---

## 4. 이미지 생성 프롬프트 설계 (Prompt Architecture)

모든 어휘 카드는 모바일 앱 UI에 최적화된 **5-Part 2D Anime 프롬프트 아키텍처**를 적용하여 생성됩니다.

### 4.1 표준 프롬프트 템플릿 (Standard Template)
```text
...
Subject: A cute 2D anime character representing [한자/표기] ([한국어 의미]). 
Visual metaphor: Clear central visual focus on [시각 연상 메타포 개념]. 
...
Constraint: NO text, NO written words.
```

### 4.2 주요 제약 및 품질 보장 조건 (Quality Constraints)
...
## 5. 생성 실행 파이프라인 (Execution Pipeline)

### 5.1 스크립트 구조
- **파이프라인 위치**: `utils/image_pipeline_v2/vertex_ai_parallel_v4_generator.py`
- **저장 디렉토리**: `assets/images/words_v4_gemini/`
- **파일명 규격**: `{level}_{clean_kanji}_{clean_korean}_v4.jpg` (예: `n3_頭痛_두통_v4.jpg`)

### 5.2 병렬 처리 가속 설계
- Python `concurrent.futures.ThreadPoolExecutor`를 활용하여 비동기 멀티스레드로 API 호출
- 수집 완료 후 Git 자동 커밋 및 GitHub 원격 저장소(`main` 브랜치) 동기화
...
2. **[진행 중]** 문서화 작성 및 GitHub 저장소 PUSH
3. **[예정]** 미생성 4,126개 어휘 비동기 병렬 자동 수집 가동 (약 1시간 30분 소요 예정)
